...
async function watch(keepVid){
  ...
  try{
    pc = new RTCPeerConnection(); // LAN only, direct host candidates
    ...
    log('offer sent (' + myCands + ' local candidates), waiting for server answer...');
      ...
      try{
        ...
        if(r.ok){
          ...
          if(data.sdp){
            ...
            log('answer received (' + remCands + ' remote candidates)');
